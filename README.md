...
- A DEX adheres to a fixed order expiry. If an order expires before being filled or canceled, the DEX will send an `r2` refund transaction back to the user's wallet address which will return the unfilled amount of the original transaction minus any blockchain transaction fees incurred by the DEX.
